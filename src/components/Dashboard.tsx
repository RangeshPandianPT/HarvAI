...
import type { Farmer, WeatherAlert, MarketPrice, CropJournalEntry } from '@/services/mockBackend';
...
import { BackendStatusCard } from './BackendStatus';
...
const Dashboard = () => {
  ...
  const fetchFarmerProfile = async () => {
    try {
      ...
      const { data, error } = await backendService.getFarmerProfile(user.id);

      if (error && error.message !== 'Farmer not found') {
        throw error;
      }
  ...
  const fetchWeatherAlerts = async () => {
    try {
      ...
      const { data, error } = await backendService.getWeatherAlerts(farmer.state, farmer.district);
  ...
  const fetchMarketData = async () => {
    try {
      ...
      const { data, error } = await backendService.getMarketPrices(farmer.state, farmer.district);
  ...
  const fetchJournalEntries = async () => {
    try {
      ...
      const { data, error } = await backendService.getCropJournalEntries(farmer.id);
  ...
  const handleSoilAnalysis = async () => {
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        const { data, error } = await backendService.performSoilAnalysis(farmer.id!, base64);
  ...
  const fetchSoilAnalysisResults = async () => {
    try {
      ...
      const { data, error } = await backendService.getSoilAnalyses(farmer.id);
  ...
  const fetchSatelliteResults = async () => {
    try {
      ...
      const { data, error } = await backendService.getSatelliteAnalyses(farmer.id);
  ...
  const handleSatelliteAnalysis = async () => {
    ...
    try {
      const { data, error } = await backendService.performSatelliteAnalysis(
        farmer.id,
        parseFloat(latitude),
  ...
  const addJournalEntry = async () => {
    ...
    try {
      const { error } = await backendService.addCropJournalEntry(farmer.id, {
        ...newEntry,
        farmer_id: farmer.id,
    ...
    <div className="min-h-screen bg-gradient-to-br from-growth-light via-background to-water-blue/20">
      <div className="container mx-auto p-6">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="animate-fade-in">
          <TabsList className="grid w-full grid-cols-8 h-16 bg-white/80 backdrop-blur-sm shadow-soft border border-border/50">
            ...
            <TabsTrigger value="weather" className="flex-col h-14 data-[state=active]:bg-gradient-to-b data-[state=active]:from-sky-blue data-[state=active]:to-water-blue data-[state=active]:text-white">
              <CloudRain className="h-5 w-5 mb-1" />
              <span className="text-xs font-medium">{t('weatherTitle')}</span>
            </TabsTrigger>
